...
using WalletConnectSharp.Core.Models.BNB;
...
namespace WalletConnectSharp.Core
{
    public class WalletConnectSession : WalletConnectProtocol
    {
        ...
        public WalletConnectSession(ClientMeta clientMeta, string bridgeUrl = null, ITransport transport = null, ICipher cipher = null, /*int chainId = 56*/ int chainId = 1, EventDelegator eventDelegator = null) : base(transport, cipher, eventDelegator)
        {
            if (clientMeta == null)
            {
        ...
        public override async Task Disconnect()
        {
            ...
            Debug.LogError("DISCONNET WALLET");
        ...
        public async Task<string> BNBSign(string address, string message)
        {
            EnsureNotDisconnected();
            ...
            var request = new BnbSign(address, message);

            var response = await Send<BnbSign, EthResponse>(request);
        ...
        public async Task<string> EthSwitchNetwork(params SwitchChainData[] chainData)
        {
            EnsureNotDisconnected();
        ...
        public async Task<string> BNBApprove(params TransactionData[] transaction)
        {
            EnsureNotDisconnected();
            
            var request = new BnbApprove(transaction);
            
            var response = await Send<BnbApprove, EthResponse>(request);

            return response.Result;
        }

        public async Task<string> BNBSwapTokenToCoin(params TransactionData[] transaction)
        {
            EnsureNotDisconnected();
            
            var request = new BnbSwapTokenToCoin(transaction);
            
            var response = await Send<BnbSwapTokenToCoin, EthResponse>(request);
