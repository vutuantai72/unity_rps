...
public class ObjectPooler : MonoBehaviour
{
    ...
    public GameObject GetPooledObject(int index)
    {
        ...
        {
            GameObject @object = (GameObject)Instantiate(itemsToPool[index].objectToPool);
            @object.transform.parent = this.transform;
            //@object.transform.localScale = Vector3.one * 30000;
            @object.SetActive(false);
            pooledObjectsList[index].Add(@object);
            return @object;
        }
        return null;
    }
    ...
    private void ObjectPoolItemToPooledObject(int index)
    {
        ...
        {
            GameObject @object = (GameObject)Instantiate(item.objectToPool);
            @object.transform.parent = this.transform;
            //@object.transform.localScale = Vector3.one * 30000;
            @object.SetActive(false);
