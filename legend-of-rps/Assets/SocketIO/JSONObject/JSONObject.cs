...
public class JSONObject {
	...
	public JSONObject(float f) {
		type = Type.NUMBER;
		n = f;
		Debug.LogError("JSONObject float" + n);
	}
#else
	public JSONObject(double d) {
		type = Type.NUMBER;
		n = d;
		Debug.LogError("JSONObject double" + n);
	}
#endif
	...
	public static JSONObject Create(float val) {
		...
		obj.n = val;
		Debug.LogError("JSONObject Create float" + val);
		return obj;
	}
	public static JSONObject Create(int val) {
		...
		obj.type = Type.NUMBER;
		obj.n = val;
		Debug.LogError("JSONObject Create int" + val);
		return obj;
	}
				...
				if(string.Compare(str, "true", true) == 0) {
					...
					while(++offset < str.Length) {
						...
						if((str[offset] == ',' && depth == 0) || depth < 0) {
							...
							if (inner.Contains("{\"jackpot\":"))
								gameService.jsonJackpot.Value = inner;
							if (inner.Contains("{\"coin\":"))
								gameService.jsonCoin.Value = inner;
							if (inner.Length > 0) {
								if(type == Type.OBJECT)
									keys.Add(propName);
